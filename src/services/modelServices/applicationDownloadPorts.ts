...
import { mobileImageDownloadMetadata, mobileTextDownloadMetadata } from './modelDownloadRequests';
import {
  ...
  return {
    ...
    async prepare(signal) {
      ...
      for (const [index, artifact] of installs.entries()) {
        ...
        installed.push({ artifactId: artifact.artifactId, localName: artifact.localName });
      }
      if (signal.aborted) throw cancelled();
...
function publicMetadata(record: Readonly<PersistedModelDownload>): string | undefined {
...
async function beginTextFinalization(
  ...
  const metadata = mobileTextDownloadMetadata(publicMetadata(record));
  ...
  const primary = record.manifest.artifacts.find(artifact => artifact.role === 'primary');
  ...
  const restoreTextRegistry = async (models: readonly typeof previousModels[number][]) => {
    await saveModelsList([...models]);
    useAppStore.getState().setDownloadedModels([...models]);
  };
  const installs = record.manifest.artifacts.map(artifact => {
    const finalName = artifact.role === 'mmproj'
  ...
  return {
    recoveryState: transaction.recoveryState,
    async prepare(signal) {
      if (!(await RNFS.exists(modelsDirectory))) await RNFS.mkdir(modelsDirectory);
      const installed: Array<{ artifactId: string; localName: string }> = [];
      ...
      const model = await buildDownloadedModel({
        ...
        file: metadata.file,
        resolvedLocalPath: documentFiles.pathFor(`${APP_CONFIG.modelStorageDir}/${primary.name}`),
...
async function restoreImageRegistry(models: readonly ONNXImageModel[]): Promise<void> {
...
async function beginImageFinalization(
  ...
  const descriptor = imageDownloadDescriptorFromMetadata(record.manifest.modelId, metadata);
  const root = `${RNFS.DocumentDirectoryPath}/image_models`;
  const destination = `${root}/${descriptor.id}`;
  const prepared = `${RNFS.DocumentDirectoryPath}/offgrid-download-staging/${encodeURIComponent(record.manifest.id)}/prepared-image`;
  ...
  }, undefined, restoreImageRegistry);
  return {
    recoveryState: transaction.recoveryState,
    async prepare(signal) {
      if (signal.aborted) throw cancelled();
      ...
      if (metadata.imageDownloadType === 'zip') {
        ...
        for (const artifact of record.manifest.artifacts) {
          ...
          await RNFS.copyFile(documentFiles.pathFor(artifact.localName), target);
        }
        await RNFS.writeFile(`${prepared}/_ready`, '', 'utf8');
      }
      if (descriptor.backend === 'mnn' || descriptor.backend === 'qnn') {
        const integrity = await validateImageModelDir(prepared, descriptor.backend);
        if (!integrity.complete) throw new Error(`Image model is incomplete: ${integrity.missing.join(', ')}`);
      }
      ...
      const resolvedModelPath = descriptor.backend === 'coreml'
      ...
      return {
        artifacts: record.manifest.artifacts.map(artifact => ({
          artifactId: artifact.id,
          localName: metadata.imageDownloadType === 'zip'
            ? `image_models/${descriptor.id}`
            : `image_models/${descriptor.id}/${safeImageRelativePath(artifact.name)}`,
        })),
      };
    },
    commit: () => transaction.commit(),
...
function createFinalizer(managed?: MobileManagedArtifactIO): DownloadFinalizePort {
  return {
  async recover(input) {
    if (input.download.manifest.kind === 'voice') {
      if (!managed) throw new Error('This build does not support voice model downloads.');
      await managed.recoverFinalization(input);
      return;
    }
    const restoreTextRegistry = async (models: readonly DownloadedModel[]) => {
      await saveModelsList([...models]);
      useAppStore.getState().setDownloadedModels([...models]);
    };
    const transaction = new DownloadInstallTransaction(
  ...
  async begin(record) {
    ...
    throw new Error(`Mobile download finalizer does not support ${record.manifest.kind} yet.`);
  },
  };
}
...
export function createMobileApplicationDownloadPorts(
  ...
  const transfers = compositeDownloadTransferPort(nativeDownloadTransferAdapter, managed);
  return {
  ports: {
  ...
  sources: {
    resolve: async request => {
      const text = request.modelType === 'text'
        ...
        artifacts: [
          {
            ...
            url: text.file.downloadUrl || huggingFaceService.getDownloadUrl(text.repositoryId, text.file.name),
          ...
          ...(text.file.mmProjFile ? [{
            ...
            url: text.file.mmProjFile.downloadUrl || huggingFaceService.getDownloadUrl(text.repositoryId, text.file.mmProjFile.name),
      ...
      if (request.modelType === 'image') {
        ...
        if (!metadata) throw new Error('Image download metadata is invalid.');
        const descriptor = imageDownloadDescriptorFromMetadata(request.modelId, metadata);
        const sourceFiles = metadata.imageModelHuggingFaceFiles ?? metadata.imageModelCoremlFiles;
        return {
          displayName: metadata.imageModelName,
          catalogEntry: true,
          artifacts: metadata.imageDownloadType === 'zip'
            ? [{
  ...
  imageOnboardingComplete: () => useAppStore.getState().onboardingChecklist.triedImageGen,
  };
}
