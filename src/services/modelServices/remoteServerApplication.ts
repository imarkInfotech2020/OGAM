...
function readConfiguration(): RemoteServerConfiguration {
  ...
  return { version: 1, activeServerId: selectedRemoteServerId('text'), servers: state.servers };
}
...
export const mobileRemoteServerPorts: Omit<RemoteServerApplicationPorts, 'select'> = {
    ...
    scan: (onFound, onProgress) => discoverLANServers(undefined, onFound, onProgress),
