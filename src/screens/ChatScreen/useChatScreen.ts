...
export const useChatScreen = () => {
  ...
  useEffect(() => {
    const unsub1 = imageGenerationService.subscribe(state =>
  ...
  const handleToggleTool = (toolId: string) => {
    ...
    useAppStore
      .getState()
      .updateSettings({
        enabledTools: cur.includes(toolId)
          ? cur.filter((id: string) => id !== toolId)
          : [...cur, toolId],
      });
  };
