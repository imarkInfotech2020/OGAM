...
| PM9 | **No receive-side consent. The receiver cannot accept or reject anything.** Ambient sharing gives the SENDER an Off/Ask/Auto policy per category, but nothing on the receiving device decides whether to take what arrives: `SinkRegistry.createSink` dispatches purely on MIME type (`factory ? factory(...) : null`) and every app factory (model, knowledge document, shared file) builds a sink unconditionally. A paired device can push a 5.6 GB model, a file or a document and the receiver stores it with no policy, no prompt and no per-device rule. The refusal MECHANISM already exists - returning `null` sends `file_reject` "transfer refused" - it is simply never wired to a user setting. The brief requires the opposite: "Each channel needs its own policy: Is it enabled? Is it automatic or user-approved? Which devices receive it?" | real gap, user-reported 2026-07-30. Shape: a receive policy per category (Off / Ask / Auto) plus a per-device rule, consulted inside `createSink` so ONE gate covers every channel on every platform; "Ask" needs an approval prompt plus a queue for offers arriving while the app is closed. Matrix needs rows for reject, ask-then-accept, ask-then-decline. |
