...
jest.mock('@react-native-documents/picker', () => ({
  ...
  keepLocalCopy: jest.fn(() =>
    Promise.resolve([{ status: 'success', localUri: 'file:///mock/doc.pdf' }]),
  ),
}));
...
describe('ProjectDetailScreen basic rendering (real composition)', () => {
  ...
  beforeAll(async () => {
    ...
    nativeBoundary = installNativeBoundary({
      download: true,
      fs: true,
      llama: true,
    });
    ...
    ({
      ProjectDetailScreen: RealProjectDetailScreen,
    } = require('../../../src/screens/ProjectDetailScreen'));
    ({
      ProjectsScreen: RealProjectsScreen,
    } = require('../../../src/screens/ProjectsScreen'));
    ({
      ProjectEditScreen: RealProjectEditScreen,
    } = require('../../../src/screens/ProjectEditScreen'));
    ({
      ChatScreen: RealChatScreen,
    } = require('../../../src/screens/ChatScreen'));
    fixture = await (
    ...
    return rtl.render(
      ...
      >
        <Stack.Navigator>
          ...
          <Stack.Screen
            name="ProjectDetail"
            component={RealProjectDetailScreen}
          />
  ...
  const renderProjectWithConversation = (title: string) => {
    ...
    rtl.act(() => {
      conversationId = useChatStore
        .getState()
        .createConversation('external-model', title, mockRouteProjectId);
    });
  ...
  const renderProjectWithActiveRemoteText = async (realNavigation = false) => {
    ...
    const discovered = await fixture.application.models.discoverRemoteServers(
      saved.value.id,
    );
    if (!discovered.ok)
  ...
  const renderProjectWithRemoteInventory = async (realNavigation = false) => {
    ...
    await startModelDownload('author/inventory', {
      ...
      quantization: 'Q4_K_M',
      downloadUrl:
        'https://huggingface.co/author/inventory/resolve/main/inventory.Q4_K_M.gguf',
    });
    ...
    await rtl.waitFor(async () => {
      ...
      expect(
        snapshot.inventory.some(model => model.id.includes('author/inventory')),
      ).toBe(true);
    });
    const project = createProject();
    return realNavigation
  ...
  it('does not show project description in header', () => {
    expect(
      renderProject().queryByText('A test project description'),
    ).toBeNull();
  });
  ...
  it('opens the selected project in the real editor', async () => {
    ...
    expect(view.getByTestId('project-edit-system-prompt').props.value).toBe(
      'Be helpful',
    );
  });
  ...
  it('does not show conversations from other projects', () => {
    ...
    rtl.act(() => {
      useChatStore
        .getState()
        .createConversation('external-model', 'Other Project Chat', other.id);
    });
  ...
  it('shows correct chat count in stats', () => {
    ...
    rtl.act(() => {
      useChatStore
        .getState()
        .createConversation('external-model', 'Chat 2', mockRouteProjectId);
    });
  ...
  it('opens the real Chat screen when a conversation is tapped', async () => {
    ...
    const conversationId = useChatStore
      .getState()
      .createConversation('external-model', 'Tappable Chat', project.id);
    const view = renderProjectHistory(project.id);
    rtl.fireEvent.press(view.getByText('Tappable Chat'));
    expect(useChatStore.getState().getActiveConversation()?.id).toBe(
      conversationId,
    );
  ...
  it('disables New button when no models available', () => {
    expect(
      renderProject().getByRole('button', {
        name: 'New',
        disabled: true,
      }),
    ).toBeTruthy();
  ...
  it('opens the real project chat when "New" is pressed', async () => {
    ...
    const selectedModelId =
      fixture.application.models.snapshot().active.text?.model?.id;
    ...
    const conversation = useChatStore
      .getState()
      .conversations.find(item => item.projectId === mockRouteProjectId);
  ...
  it('does not render description when null', () => {
    expect(
      renderProject(null).queryByText('A test project description'),
    ).toBeNull();
  });
  ...
  it('starts a chat when only an inventory model is available', async () => {
    ...
    const inventoryModelId = fixture.application.models
      .snapshot()
      .inventory.find(model => model.modality === 'text')?.id;
  ...
  it('disables Add while indexing and re-enables it after completion', async () => {
    ...
    const heldRead = new Promise<string>(resolve => {
      releaseRead = resolve;
    });
    ...
    try {
      ...
      expect(
        view.getByRole('button', { name: 'Add', disabled: true }),
      ).toBeTruthy();
      releaseRead('held content');
      expect(await view.findByText('held.txt')).toBeTruthy();
      expect(
        view.getByRole('button', { name: 'Add', disabled: false }),
      ).toBeTruthy();
    } finally {
  ...
  it('indexes every selected file in a multi-file import', async () => {
    ...
    DocumentPicker.keepLocalCopy
      .mockResolvedValueOnce([
        { status: 'success', localUri: `file://${base}/file1.txt` },
      ])
      .mockResolvedValueOnce([
        { status: 'success', localUri: `file://${base}/file2.txt` },
      ]);
    rtl.fireEvent.press(view.getByTestId('kb-add-document'));
