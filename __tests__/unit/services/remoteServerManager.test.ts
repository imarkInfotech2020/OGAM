jest.mock('react-native-keychain', () => {
  ...
  return {
    ...
    setGenericPassword: jest.fn(async (_user, password, options) => {
      values.set(options.service, password); return true;
    }),
    ...
    resetGenericPassword: jest.fn(async options => {
      values.delete(options.service); return true;
    }),
  };
});
...
describe('Mobile remote-server application composition', () => {
  ...
  it('routes add, endpoint deduplication, and update through Shared authority', async () => {
    const first = await remoteServerManager.addServer({
      name: 'Desktop', endpoint: 'http://desktop.local:7878', provider: 'openai-compatible',
    });
    const duplicate = await remoteServerManager.addServer({
      name: 'Duplicate', endpoint: 'http://desktop.local:7878/', provider: 'openai-compatible',
    });
  ...
  it('keeps credentials outside the public Zustand projection', async () => {
    const server = await remoteServerManager.addServer({
      name: 'Cloud', endpoint: 'https://models.example.com/v1',
      provider: 'openai-compatible', apiKey: 'secret',
    });
    expect(useRemoteServerStore.getState().servers[0]).not.toHaveProperty('apiKey');
    expect(await remoteServerManager.getServerWithApiKey(server.id)).toMatchObject({ apiKey: 'secret' });
  });
