...
describe('tool-call grammar — single source, stripper and stream filter agree', () => {
  ...
  it.each(combos)('stripControlTokens removes a block opened by %o', ({ open, close }) => {
  ...
  it.each(combos)('ToolCallTokenFilter suppresses a block opened by %o (whole token)', ({ open, close }) => {
  ...
  it.each(combos)('ToolCallTokenFilter suppresses a block opened by %o split char-by-char', ({ open, close }) => {
  ...
  it('strips an UNCLOSED tool-call opener at end of stored content (EOS mid-call)', () => {
    expect(stripControlTokens('Working on it.<tool_call:get_weather{"city":"NY')).toBe('Working on it.');
    expect(stripControlTokens('Working on it.<|tool_call>call:x{')).toBe('Working on it.');
  });
});
