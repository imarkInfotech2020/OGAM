...
beforeAll(async () => {
  ...
  boundary.fs!.seedFile('/models/sd/model.mnn', 8 * 1024 * 1024);
...
async function generateWithSettings(settings: Record<string, unknown>) {
  ...
  boundary.diffusion.module.getLoadedModelPath.mockResolvedValue('/models/sd');
  await localDreamGeneratorService.loadModel('/models/sd', 4, {});
  await applicationFixture!.refreshModels();
...
describe('image gen meta — UI red-flow (the size/guidance you set is what runs)', () => {
  it('Q1 (guard): an unsupported 128 size correctly floors to 256x256', async () => {
    const view = await generateWithSettings({
      imageWidth: 128,
      imageHeight: 128,
    });
