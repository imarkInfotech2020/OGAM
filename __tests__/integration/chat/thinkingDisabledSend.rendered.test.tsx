...
describe('Mobile chat send with Thinking disabled', () => {
  it('disables Thinking in quick settings and shows the clean reply to the next message', async () => {
    ...
    await h.rtl.waitFor(() => {
      expect(
        h.rtl.within(view.getByTestId('quick-thinking-toggle')).getByText('ON'),
      ).toBeVisible();
    });
    ...
    await h.rtl.waitFor(() => {
      expect(
        h.rtl.within(view.getByTestId('quick-thinking-toggle')).getByText('OFF'),
      ).toBeVisible();
    });
