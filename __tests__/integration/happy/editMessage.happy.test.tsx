...
describe('happy — edit a message via the real action menu (heavy entry point)', () => {
  ...
  it.each(['longpress', 'dots'] as const)(
    '%s → Edit → change text → SAVE & RESEND re-runs generation',
    async via => {
      const h = await setupChatScreen({ engine: 'litert' });
      h.render();

      await h.send('what is the capital of span', {
        content: 'The capital of Spain is Madrid.',
      });
      await h.rtl.waitFor(() => {
        expect(
          h.view!.queryByText(/The capital of Spain is Madrid\./),
        ).not.toBeNull();
      });

      // User fixes the typo and resends via the real Edit gesture (opened via this affordance).
      await h.editLastUserMessage(
      ...
      await h.rtl.waitFor(() => {
        expect(
          h.view!.queryByText(/Madrid is the capital of Spain\./),
        ).not.toBeNull();
      });
    },
  );
